...
class Database:
    ...
    async def first_setup(self):
        await self.create_table('credentials', id='varchar', name='TEXT UNIQUE PRIMARY KEY', value='varchar',
        ...
        await self.create_table('timers', id='varchar PRIMARY KEY', active='BOOL DEFAULT True', channel='TEXT', name='TEXT',
    ...
    async def get_credentials(self, name):
        async with aiosqlite.connect(self.db_path) as db:
            db.row_factory = Row
            async with db.execute("SELECT * FROM credentials WHERE name = ?", (name, )) as cursor:
                async for row in cursor:
                    return dict(row)['value']



    async def insert_or_update(self, table_name, **kwargs):
        for key, value in kwargs.items():
            id = utils.id_generator.generate_id()
            await self.execute(f'INSERT OR REPLACE INTO {table_name} (id, name, value) VALUES (?, ?, ?)', (id, key, value))
